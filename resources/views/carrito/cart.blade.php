            ...
            <div class="p-6">
                            ...
                            <tr>
                                <td class="px-4 py-4">
                                    <div class="flex items-center">
                                        <img src="{{ $item->attributes->imagen1 ?? '#' }}" alt="{{ $item->name }}" class="w-16 h-16 object-cover rounded">
                                        <div class="ml-4">
                                ...
                                <td class="px-4 py-4 text-sm text-gray-500">
                                    <form action="{{ route('updateCantidad') }}" method="POST" class="flex items-center">
                                        @csrf
                                        <input type="hidden" name="id" value="{{ $item->id }}">
                                        <button type="submit" name="accion" value="disminuir" class="text-gray-500 hover:text-gray-700 px-2">-</button>
                                        <span class="mx-2">{{ $item->quantity }}</span>
                                        <button type="submit" name="accion" value="incrementar" class="text-gray-500 hover:text-gray-700 px-2">+</button>
                                    </form>
                                </td>
                                <td class="px-4 py-4 text-sm font-medium text-gray-900">${{ number_format($item->price * $item->quantity, 2) }}</td>
                ...
                <div class="mt-8 border-t border-gray-200 pt-6">
                    <div class="flex justify-between">
                        ...
                        <span class="text-base font-medium text-gray-900">${{ number_format($cart->getSubTotal(), 2) }}</span>
                    </div>
                    <div class="flex justify-between mt-2">
                        <span class="text-base font-medium text-gray-900">Impuestos:</span>
                        <span class="text-base font-medium text-gray-900">$0.00</span> {{-- darryldecode no calcula impuestos automáticamente --}}
                    </div>
                    <div class="flex justify-between mt-4 border-t border-gray-200 pt-4">
                        <span class="text-lg font-bold text-gray-900">Total:</span>
                        <span class="text-lg font-bold text-gray-900">${{ number_format($cart->getTotal(), 2) }}</span>
                    </div>
                </div>
